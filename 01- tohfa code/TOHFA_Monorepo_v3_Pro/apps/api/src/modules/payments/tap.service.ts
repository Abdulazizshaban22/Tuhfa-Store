...
@Injectable() export class TapService {
  ...
  async createCharge(amountSar: number, metadata: any, method: Method){
    ...
    const payload = {
      amount: amountSar, currency: 'SAR', threeDSecure: true, customer_initiated: true,
      description: 'TOHFA charge', metadata, customer: metadata?.customer || undefined,
    ...
    const res = await axios.post(`${this.base}/charges`, payload, {
      headers: { Authorization: `Bearer ${this.secret}`, 'Content-Type': 'application/json', 'Idempotency-Key': idempotency }, timeout: 10000,
    });
    const d = res.data; return { provider:'tap', status: d.status, chargeId: d.id, transactionUrl: d?.transaction?.url, response: d?.response };
  }
}
