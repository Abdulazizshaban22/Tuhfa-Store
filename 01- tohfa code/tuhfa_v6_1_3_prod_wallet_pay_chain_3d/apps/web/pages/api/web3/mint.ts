...
export default async function handler(req: NextApiRequest, res: NextApiResponse){
  ...
  if(!to || !uri) return res.status(400).json({ error:'to/uri required' });
  try{ const tx = await c.safeMint(to, uri); return res.status(200).json({ ok:true, tx: tx.hash }); }
  catch(e:any){ return res.status(500).json({ error: e.message || 'mint failed' }); }
}
